    ...
    while(!ifile.fail() && !error_) {
        ...
        if(state == FIND_SECTION) {
            ...
            if((line.size() > 3) && line[0] == '<' && line[1] == '/' &&
             line[line.size()-1] == '>') {
                map<string, SectionParser*>::iterator it = 
                parsers_.find(sectionName);
                if(it != parsers_.end()) {
                    error_ = it->second->parse(sectionText, ds, 
                        startLineNo, errorMsg_);
                    if(error_) {
                        lineno_ = startLineNo;
                    }
                    ...
                    cerr << "No section parser available for " ;
                    cerr<< sectionName << "..skipping!" << endl;
                }
                state = FIND_SECTION;
#ifdef DEBUG
        ...
        if(error_) {
            ...
            cerr << lineno_ << ": " << errorMsg_ << endl;
        }
        lineno_++;
    }
    if(!error_) {
        for(map<string, SectionParser*>::iterator it =
         parsers_.begin();
                it != parsers_.end();
                ++it)
        {
...
ProductSectionParser::~ProductSectionParser()
{
    for(map<string, ProductParser*>::iterator it = 
        prodParsers_.begin();
            it != prodParsers_.end();
            ++it) {
        delete it->second;
    ...
    std::string& errorMsg)
{
    ...
    while(getline(is, line)) {
        ...
        string category;
        ss >> category;
        map<string,ProductParser*>::iterator it =
         prodParsers_.find(category);
